...
import { APP_CONFIG, isContentAllowed } from '@/config/appConfig.ts';
...
export const CHALLENGE_GROUP_LABELS: Record<ChallengeGroup, string> = {
  grounding: 'Grounding & Breath',
  self_compassion: 'Self-Compassion',
  connection: 'Connection',
  recovery: 'Recovery',
  focus: 'Focus & Momentum',
  habits: 'Healthy Habits',
  faith: 'Faith',
};

/** Order groups are rendered in - gentlest first. */
const GROUP_ORDER: ChallengeGroup[] = [
  'grounding',
  'self_compassion',
  'connection',
  'recovery',
...
export type Challenge = {
  ...
  icon?: string;
  /** Explicitly religious - hidden unless faith content is enabled. */
  faith?: boolean;
};
...
/** Every challenge the current config allows. Use this instead of CHALLENGES. */
export const getVisibleChallenges = (): Challenge[] =>
  CHALLENGES.filter((challenge) => isContentAllowed(Boolean(challenge.faith)));

/**
 * The short, curated set shown by default. Falls back to the full (visible)
 * library when the config exposes everything.
 */
export const getCuratedChallenges = (): Challenge[] => {
  const visible = getVisibleChallenges();
  const curatedIds = APP_CONFIG.curatedChallengeIds;

  if (!curatedIds) {
    return visible;
  }

  return visible.filter((challenge) => curatedIds.includes(challenge.id));
};

export const groupChallenges = (
  challenges: Challenge[]
): { group: ChallengeGroup; label: string; challenges: Challenge[] }[] =>
  GROUP_ORDER.map((group) => ({
    group,
    label: CHALLENGE_GROUP_LABELS[group],
    challenges: challenges.filter((challenge) => challenge.group === group),
  })).filter((section) => section.challenges.length > 0);

export const getChallengesByCoachType = (coachType: QuoteType): Challenge[] => {
  return getVisibleChallenges().filter(challenge =>
